defmodule ElixirPluginTemplate.MixProject do
  ...
  defp adjust(release) do
    for {app, props} <- release.applications,
        app in [:elixir, :iex] or props[:otp_app?] do
      dir = "#{app}-#{props[:vsn]}"
  ...
  defp emqx_tar(release) do
    ...
    lib_dirs =
      Enum.reduce(release.applications, [], fn {app, config}, acc ->
        if auto_included_app?(app, config) do
          acc
        else
          dir = "#{app}-#{config[:vsn]}"
          [dir | acc]
  ...
  defp auto_included_app?(app, config) do
    app in [:elixir, :iex] or config[:otp_app?]
  end
